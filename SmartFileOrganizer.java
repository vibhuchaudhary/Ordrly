...
public class SmartFileOrganizer extends JFrame {
    ...
    public SmartFileOrganizer() {
        ...
        // Attempt to load a font that strongly supports emojis
        ...
        for (String fontName : emojiFontCandidates) {
            ...
            if (testFont.canDisplay('\uD83D') && testFont.canDisplay('\uDE00')) { // Check if it can display basic emoji characters
                emojiFont = testFont;
                break;
            }
        }
        if (emojiFont == null) {
            emojiFont = new Font("Arial", Font.PLAIN, 22); // Fallback font
            System.err.println("Warning: No suitable emoji font found. Emojis may not render correctly.");
        }
        regularFont = new Font("Segoe UI", Font.BOLD, 14); // Default regular font
        ...
        selectFolderButton.setFont(emojiFont.deriveFont(Font.BOLD, 14)); // Use emoji font for the button too
        ...
        protected void done() {
            ...
            if (!isCancelled()) {
                try {
                    ...
                    String messageToShow = finalErrorMessage; // Effectively final variable
